...
function topKFrequent(nums: number[], k: number): number[] {
  const quantities = nums.reduce<{ [key: number]: number }>((acc, val) => {
    acc[val] = (acc[val] || 0) - 1;

    return acc;
  }, {});

  const queue = new PriorityQueue<{ val: number; count: number }>((item1, item2) => {
  ...
  Object.keys(quantities).forEach((key) => {
    queue.enqueue({ val: +key, count: quantities[key] });
  });
