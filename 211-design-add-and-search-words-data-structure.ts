namespace Problem211 {
  class WordDictionaryNode {
    ...
    getOrCreateChild(character: string, isEndOfWord: boolean): WordDictionaryNode {
      const index = this.characterToIndex(character);
  ...
  class WordDictionary {
    ...
    private match(word: string[], index: number, node: WordDictionaryNode | undefined): boolean {
      if (!node) {
        return false;
      }
